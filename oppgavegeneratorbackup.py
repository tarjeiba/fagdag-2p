...
# - TODO :: Den nye læreren: alderen til den nye læreren må være et helt tall. 
# - TODO :: Diagramtolkning: beholder a-versjonen, men gjør slik at den ene grafen får litt større spredning. 
# - TODO :: Histogrammet: fin!
# - TODO :: Kumulativ match: fjern den ene m-en i KUMMULATIV :-) 
# - TODO :: Foreslå visualisering og Skisser i samme oppgave: oppgavetekst: "velg to av punktene under og illustrerer dem i hvert sitt diagram. Begrunn valget av diagram." 
...
if solutions:
    template_file_name = 'oppgave-mal-fasit.org'
else:
...
def populate_freq_table(level="a", seed_string="test", sol=False, dictkey=None):
    if level == "a":
        return populate_freq_table_easy(rnd.randint(15,25), seed_string=seed_string, sol=sol, dictkey=dictkey)
    else:
        return populate_freq_table_hard(rnd.randint(25, 50), seed_string=seed_string, sol=sol, dictkey=dictkey)
 
def populate_freq_table_easy(n, t=6, seed_string="test", sol=False, dictkey=None):
    ...
    if sol:
        ...
        repl[dictkey] = ("Gjennomsnittet er {0:.2f}.\n"
                         "Medianen er {1}.\n"
                         "Typetallet er {2}.\n"
...
def populate_freq_table_hard(n=None, t=8, seed_string="test", sol=False, dictkey=None):
...
def nplusone_average(level="a", seed_string="test", sol=False, dictkey=None):
    ...
    res = rnd.randint(22, 45)
    n = rnd.randint(10, 20)
    init_average = rnd.randint(32, 40)
 
    new_average = (n*init_average + res) / (n + 1)
 
    value = ("På en skole jobber det i utgangspunktet {0} realfagslærere, "
             "som da hadde en gjennomsnittsalder på {1:.1f} år. Det begynner så en "
             "ny lærer på skolen, og brått blir gjennomsnittsalderen {2:.1f} år.\n\n"
             "Hva er aldereden til den nye læreren?").format(n, init_average, new_average)
 
    if sol:
        repl[dictkey] = 'Fasiten er {}.'.format(res)
...
def create_assignment(group="testgroup", student=1, level="a", template=template, replacements=None):
    ...
    replacements['{HISTOGRAMADD}'] = histogram_addition(
        level=level)
    replacements['{ELEVNUMMER}'] = str(
        student)
    replacements['{KLASSE}'] = str(
        group)
    replacements['{LEVEL}'] = str(
        level)
